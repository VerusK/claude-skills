...
Дата: 2026-09-24. Статус: согласован в kickoff, уточнён по plan-review, раунды 1 и 2 (§7).
...
## 7. Изменения по plan-review (раунды 1 и 2)
...
- **§0:** путь спеки разрешается через `realpath` по самому файлу, поэтому симлинк на файл вне репозитория ловится (`auto`).
- **§1:** промпт второго раунда просит проверить всю изменённую спеку: старые нерешённые находки и новые, которые внесли правки.
- **§4, §6:** коммитится только спека (`git add -- "$SPEC" && git commit … -- "$SPEC"`), чужие изменения в checkout не попадают.
- **`scripts/reviewer.sh`:** отчёт засчитывается, только если маркер стоит последней непустой строкой, на обоих путях (`auto`). Терминал Orca создаётся в проверяемом репозитории (`--worktree path:<repo>`); репозиторий, которым Orca не управляет, уходит в `codex exec` (`auto`).
- **Приёмка:** к шести сценариям добавлены пять принудительных: CLEAR без коммита, выход 3, неудачный запуск, судья недоступен, второй раунд по P0/P1 (`auto`).

Ревью ветки (раунд 1, выбор пользователя): промпт ревьюера `spec-review` по-прежнему запрещает читать `~/.codex/`, `~/.agents/` и `.codex/skills/`, но файлы `agents/` и `.claude/` этого репозитория можно читать как данные, если спека на них ссылается; сборщик промпта больше не выкидывает их из списка файлов. Рамка `plan-review/reviewer.md` не меняется.
